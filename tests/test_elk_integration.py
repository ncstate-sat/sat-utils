...
def log_stuff(request):
    with patch('sat.logs.bulk') as bulk_patch:
        ...
        elastic_handler = ElasticClientHandler(client=mock_client, index_name='test', document_labels={'app': 'test'}, level=logging.INFO, batch_size=1, batch_time=10.)
...
def test_bulk_handler():
    ...
    with patch('sat.logs.bulk') as bulk_patch:
        mock_client = MagicMock()
        elastic_handler = ElasticClientHandler(client=mock_client, index_name='test', document_labels={'app': 'test'}, level=logging.INFO, batch_size=3, batch_time=1000.)
        elastic_handler.setLevel(logging.INFO)
        test_formatter = logging.Formatter('%(message)s')
        elastic_handler.setFormatter(test_formatter)
        logger = logging.getLogger(__name__)
...
def test_bulk_not_called():
    ...
    with patch('sat.logs.bulk') as bulk_patch:
        mock_client = MagicMock()
        elastic_handler = ElasticClientHandler(client=mock_client, index_name='test', document_labels={'app': 'test'}, level=logging.INFO, batch_size=10, batch_time=1000.)
        elastic_handler.setLevel(logging.INFO)
        test_formatter = logging.Formatter('%(message)s')
        elastic_handler.setFormatter(test_formatter)
        logger = logging.getLogger(__name__)
